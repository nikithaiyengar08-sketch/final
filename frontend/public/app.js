...
// ===== VIBETRIP SMART TRAVEL LAYER =====

function getTravelMonth() {
  const picker = document.querySelector("#travelMonth");
  if (!picker?.value) {
    return new Date().toLocaleString("default", { month: "long" });
  }

  return new Date(picker.value + "-01")
    .toLocaleString("default", { month: "long" });
}

function getSeasonBoost(destination) {
  const month = getTravelMonth();

  if (!destination.bestMonths) return 0;
